...
_`(Doivent être exécutées après avoir sélectionné un implant avec use <id>)`_
...
- **Exécution en mémoire :** Privilégiez `execute-assembly` et `execute-bof` pour lancer des outils et des commandes. Cela évite d'écrire des fichiers exécutables sur le disque de la cible, réduisant ainsi considérablement les chances de détection par les